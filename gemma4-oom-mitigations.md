...
| ---------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Multi-GPU sharding     | DeepSpeed ZeRO Stage 1/2/3                                 | Needs ≥2 GPUs; with N=1 the sharding stages collapse to no-ops. ZeRO-3 also can't cleanly shard bnb 4-bit tensors (in-place quantization breaks the sharding contract).    |
| Multi-GPU sharding     | PyTorch FSDP (Fully Sharded Data Parallel)                 | PyTorch-native equivalent of ZeRO-3; same single-GPU = no-op story.                                                                                                        |
| Multi-GPU sharding     | Pipeline parallelism                                       | Splits the model's layers across GPUs; needs ≥2 GPUs.                                                                                                                      |
| Multi-GPU sharding     | Tensor parallelism                                         | Splits individual layer matmuls across GPUs; needs ≥2 GPUs.                                                                                                                |
